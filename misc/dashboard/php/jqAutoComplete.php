...
ACNOCACHE;
    ...
    private function getACData()
    {
        ...
        if(strlen($this->SelectCommand) > 0 ) {
            ...
            $sql = jqGridDB::prepare($this->conn,$sqlCmd, $params, true);
            $ret = jqGridDB::execute($sql);
            $ncols = jqGridDB::columnCount($sql);
            while($row=jqGridDB::fetch_num($sql)) {
...
TARGET;
            ...
            $s .= "jQuery('".$element."').autocomplete('widget').css('font-size','11px');";
            if($this->scroll) {
