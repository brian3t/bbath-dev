...
class jqGrid
{
    ...
    protected function _getcount($sql, array $params = null, array $sumcols = null)
    {
        ...
        if (preg_match("/^\s*SELECT\s+DISTINCT/is", $sql) || preg_match('/\s+GROUP\s+BY\s+/is', $sql) || preg_match('/\s+UNION\s+/is', $sql) || substr_count(strtoupper($sql), 'SELECT') > 1) {
...
class jqGridEdit extends jqGrid
{
    ...
    protected function _afterCRUDGrid($oper)
    {
        $result = true;
        switch ($oper) {
            case 'add':
                $acnt = count($this->_addarray);
                if ($acnt > 0) {
                    for ($i = 0; $i < $acnt; $i++) {
                        $stmt   = jqGridDB::prepare($this->pdo, $this->_addarray[$i]['sql'], $this->_addarray[$i]['params']);
                ...
                break;
            case 'edit':
                $acnt = count($this->_editarray);
                if ($acnt > 0) {
                    for ($i = 0; $i < $acnt; $i++) {
                        $stmt   = jqGridDB::prepare($this->pdo, $this->_editarray[$i]['sql'], $this->_editarray[$i]['params']);
                        $result = jqGridDB::execute($stmt);
                        jqGridDB::closeCursor($stmt);
                        if (!$result) {
                            break;
                ...
                break;
            case 'del':
                $acnt = count($this->_delarray);
                if ($acnt > 0) {
                    for ($i = 0; $i < $acnt; $i++) {
                        $stmt   = jqGridDB::prepare($this->pdo, $this->_delarray[$i]['sql'], $this->_delarray[$i]['params']);
                        $result = jqGridDB::execute($stmt);
                        jqGridDB::closeCursor($stmt);
                        if (!$result) {
                            break;
            ...
            if ($stmt) {
                ...
                if ($this->trans) {
                    try {
                        ...
                        jqGridDB::execute($stmt, $binds);
                        $this->_afterCRUDGrid('add');
                    ...
                    if ($result)
                        $result = $this->_afterCRUDGrid('add');
                }
                jqGridDB::closeCursor($stmt);
            } else {
                $result = false;
        ...
        if (count($updateFields) > 0) {
            ...
            if ($stmt) {
                ...
                if ($this->trans) {
                    ...
                    $result = jqGridDB::execute($stmt, $binds);
                    if ($result)
                        $result = $this->_afterCRUDGrid('edit');
                }
                jqGridDB::closeCursor($stmt);
            }
        }
            ...
            if ($stmt) {
                if ($this->trans) {
                    try {
                        ...
                        if ($test)
                            $result = $this->_afterCRUDGrid('del');
...
class jqGridRender extends jqGridEdit
{
    ...
    protected $navOptions = array("edit" => true, "add" => true, "del" => true, "search" => true, "refresh" => true, "view" => false, "excel" => true, "pdf" => false, "csv" => false);
...
PDFE;
                    $s .= "jQuery('" . $tblelement . "').jqGrid('navButtonAdd','" . $pager . "',{caption:'',title:'Export to Pdf'," . $expdf . ", buttonicon:'ui-icon-print'});";
                }
...
CSVE;
                    $s .= "jQuery('" . $tblelement . "').jqGrid('navButtonAdd','" . $pager . "',{caption:'',title:'Export to CSV'," . $excsv . ",buttonicon:'ui-icon-document'});";
                }
            }
