<?
  ...
  if(isset($_FILES['uploadedfile']['name'])) {
    ...
    if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
		...
		$result = $db->query('SELECT s.entity_id as entity_id, b.order_number as order_number FROM bulk_process b LEFT JOIN sales_flat_order s on b.order_number = s.increment_id');
        foreach($result as $order) {
        	updateOrder($order['entity_id'], $order['order_number'], 'usps', 'Priority Mail');
		}
    } else{
