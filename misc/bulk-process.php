<?
  ...
  if(isset($_FILES['uploadedfile']['name'])) {
    ...
    if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
		...
		$result = $db->query('SELECT s.entity_id as entity_id, b.order_number as order_number, b.ship_via as ship_via, b.ship_method as ship_method FROM bulk_process b LEFT JOIN sales_flat_order s on b.order_number = s.increment_id');
        foreach($result as $order) {
	        if($order['ship_via'] == 'Stamps.com') {
		        $shipper = 'usps';
	        } elseif(strpos($order['ship_via'],'FedEx') !== false) {
		        $shipper = 'fedex';
	        } else {
		        $shipper = $order['ship_via'];
	        }
	        
	        if($order['ship_method'] == 'Priority Mail') {
		        $ship_method = 'Priority Mail';
	        } elseif(strpos($order['ship_method'],'Parcel') !== false) {
		        $ship_method = 'SmartPost';
	        } else {
		        $ship_method = $order['ship_method'];
	        }
	        
        	updateOrder($order['entity_id'], $order['order_number'], $shipper, $ship_method, 'readyshipper');
		}
    } else{
  ...
  if($_POST){
    if($_POST['fedex'] == 'yes') {
      ...
      if($result){
          ...
          updateOrder($fedex_order['entity_id'], $fedex_order['order_id'], $carrier, $fedex_order['service_type'], 'shipmanager');
    ...
    function updateOrder($entityId, $orderId, $carrier, $carrierTitle, $shipping_app) {
		global $db;
...
*/
	    ...
	    if($order) {
		    ...
		    if($shipping_app == 'readyshipper') {
				$tracking_numbers = $db->query("SELECT tracking_number FROM bulk_process WHERE order_number = " . $orderId . " GROUP BY tracking_number");
		    } else {
		    ...
		    foreach($tracking_numbers as $tracking_number) {
			    ...
			    if($carrierTitle == 'SmartPost') {
				    $track_number = substr($tracking_number['tracking_number'], 2);
			    } else {
				    $track_number = $tracking_number['tracking_number'];
			    }
			    
			    $track = Mage::getModel('sales/order_shipment_track')->addData(array('carrier_code' => $carrier, 'title' => $carrierTitle, 'number' => $track_number));
