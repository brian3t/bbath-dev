...
class Mage_Sales_Model_Order_Process {
    ...
    public function awaitingPayment($orderId)
    {
    	...
    	if( $orderStatus == 'complete' || $orderStatus == 'asn_sent') {
    		$order->setStatus('payment_expected');
	...
	public function ediInvoice($orderId)
    {
		...
		if($order->getStatus() == 'asn_sent') {
			...
			if($orderItems) {
	            ...
	            $this->ftp_put_edi_files($fileName, SAVE_LOCATION, 'Outbox/810/');
			}
    ...
    public function createAsn($orderId) {
		...
		if($order->getStatus() == 'processing' || $order->getStatus() == 'holded') {
			$orderItems = $order->getItemsCollection();
    ...
    public function ediAsn($orderId, $asn_date, $sendASN = 'YES')
    {
    	define('SAVE_LOCATION', Mage::getBaseDir('base') . '/misc/export/edi/');
    	$db2 = Mage::getSingleton('core/resource')->getConnection('core_read');
    	$admin = Mage::getSingleton('admin/session')->getUser()->getFirstname();
    	...
    	if($customer_name == 'ToyRUs') {
	    	$order_status_code = 'CC';
    	} else {
	    	$order_status_code = '';
    	}
    	
		if($order_status == 'complete' || $order_status = 'label_created' || $order_status == 'processing') {
			...
			if($orderItems) {
	        	...
	        	foreach ($orderItems as $item) {
	            	...
	            	$total_order_volume = $total_order_volume + (((int)$item->getQtyShipped() / $pack_quantity) * $pack_volume);
				}
				
				$get_asn_id = 'SELECT asn_id FROM bulk_uccs WHERE order_num = \'' . $order->getRealOrderId() . '\' order by id asc LIMIT 1';
				$asn_id = $db2->fetchOne($get_asn_id);
	        	
	        	$common = $this->getCommonAsnValues($order, $customer_name, $total_order_lading_qty, $total_order_weight, $asn_date, $asn_id, $total_order_volume);
	        	...
	        	if($ship_method != 'FDEG' && $ship_method != 'UPSN' && $ship_method != 'UPSS') {
		        	$get_tare_ucc = 'SELECT ucc FROM bulk_uccs WHERE order_num is null order by id asc LIMIT 1';
					$tare_id = $db2->fetchOne($get_tare_ucc);
					$use_ucc = 'UPDATE bulk_uccs set order_num = \'' . $order->getRealOrderId() . '\', tare_id = 1 WHERE ucc = \'' . $tare_id . '\'';
					$db2->query($use_ucc);
		        ...
		        foreach ($orderItems as $item)
		        {
					...
					{	
						/*
if($customer_id == 1115){
							$get_ucc = 'SELECT ucc FROM bulk_uccs WHERE order_num = \'' . $order->getRealOrderId() . '\' AND Sent is NULL order by id asc LIMIT 1';
							$carton_id = $db2->fetchOne($get_ucc);
							$use_ucc = 'UPDATE bulk_uccs set sent = 1 WHERE ucc = \'' . $carton_id . '\'';
							$db2->query($use_ucc);
						} else {
							$get_ucc = 'SELECT ucc FROM bulk_uccs WHERE order_num is null order by id asc LIMIT 1';
							$carton_id = $db2->fetchOne($get_ucc);
							$use_ucc = 'UPDATE bulk_uccs set order_num = \'' . $order->getRealOrderId() . '\', sent = 1 WHERE ucc = \'' . $carton_id . '\'';
							$db2->query($use_ucc);			            	
		            	}
*/
						if($sendASN == 'YES') {
							$get_ucc = 'SELECT ucc FROM bulk_uccs WHERE order_num = \'' . $order->getRealOrderId() . '\' AND Sent is NULL AND label_created is not NULL AND tare_id is NULL order by id asc LIMIT 1';
							$carton_id = $db2->fetchOne($get_ucc);
							$use_ucc = 'UPDATE bulk_uccs set sent = 1 WHERE ucc = \'' . $carton_id . '\'';
							$db2->query($use_ucc);
						} elseif($sendASN == 'NO') {
							$get_ucc = 'SELECT ucc FROM bulk_uccs WHERE order_num = \'' . $order->getRealOrderId() . '\' AND Sent is NULL AND label_created is NULL AND tare_id is NULL order by id asc LIMIT 1';
							$carton_id = $db2->fetchOne($get_ucc);
							$use_ucc = 'UPDATE bulk_uccs set label_created = 1 WHERE ucc = \'' . $carton_id . '\'';
							$db2->query($use_ucc);
						}
	            ...
	            if($sendASN == 'YES' && $order_status == 'label_created') {
		            $status = 'asn_sent';
		            $status_description = 'ASN sent by ' . $admin;
		            $file_location = 'Outbox/856/';
					$this->ftp_put_edi_files($fileName, SAVE_LOCATION, $file_location);
					$order->setStatus($status);
					$order->addStatusToHistory($status, $status_description, false);
					$order->save();
	            } elseif(($order_status == 'complete' || $order_status == 'processing') && $sendASN == 'NO') {
		            $status = 'label_created';
		            $status_description = 'Label created by ' . $admin;
		            $file_location = 'Outbox/856_LabelOnly/';
		            $this->ftp_put_edi_files($fileName, SAVE_LOCATION, $file_location);
					$order->setStatus($status);
					$order->addStatusToHistory($status, $status_description, false);
					$order->save();
	...
	function getPos($remote_location = 'Inbox/850') {
		...
		foreach ($contents as $file) {
			ftp_get($connection, Mage::getBaseDir('base') . '/misc/import/po/' . $file, $file, FTP_BINARY);
		}
		
		ftp_close($connection);
    }
    
    function getLabels($remote_location = 'Inbox/Label') {
    ...
    {	
		...
		} elseif(strpos($ship_method,'UPSS') !== false) {
			$transportation_method = 'U';
    ...
    protected function getCommonAsnValues($order, $customer_name, $total_order_lading_qty, $total_order_weight, $asn_date = NULL, $asn_id, $total_order_volume)
    {	
    	$shipmentCollection = Mage::getResourceModel('sales/order_shipment_collection')->setOrderFilter($order)->load();
    	...
    	if($customer_name != 'ToyRUs') {
			...
			foreach ($shipmentCollection as $shipment) {
				foreach($shipment->getAllTracks() as $tracknum) {
					if($tracknum->getTitle() != 'LOAD') {
						$ship_method = $tracknum->getTitle();
						$tracking_number = $tracknum->getNumber();
					} else {
						$load_id = $tracknum->getNumber();
					}
		        }
			}
		}
		...
		if(strpos($ship_method,'NLRT') !== false) {
			...
			$transportation_method = 'U';
			$transportation_carrier = 'UPS Ground';
		} elseif(strpos($ship_method, 'UYSN') !== false) {
			$transportation_method = 'M';
			$transportation_carrier = 'Unyson';
		} elseif(strpos($ship_method,'UPSS') !== false) {
			$transportation_method = 'U';
			$transportation_carrier = 'UPS Ground';
		} elseif(strpos($ship_method,'UPSN') !== false) {
		...
		//$asn_id = date('mdyhis');
		if($asn_date == NULL) {
			$asn_date = Mage::getModel('core/date')->date('m/d/Y', strtotime(date('Y-m-d H:i:s')));
		}
		...
		$common_values = array(
			...
			$asn_id,
			$asn_date,
			Mage::getModel('core/date')->date('H:i:s', strtotime(date('Y-m-d H:i:s'))),
			'',
			$shipped_date,
			'',
			$estimated_delivery_date,
    ...
    public function bolship($orderId, $ship_method, $bol, $load_id) {
		...
		if( $orderStatus == 'processing' || $orderStatus == 'pending') {
			...
			if($load_id != '') {
				$load = array();
				$load['carrier_code'] = 'Custom Value';
				$load['title'] = 'LOAD';
				$load['number'] = $load_id;
				$track = Mage::getModel('sales/order_shipment_track')->addData($load);
				$shipment->addTrack($track);
			}
			$shipment->register();
			$shipment->getOrder()->setIsInProcess(true);
    ...
    public function printbol($orderId) {
