...
class Magebuzz_Dealerlocator_Adminhtml_DealerlocatorController extends Mage_Adminhtml_Controller_action
{
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('dealerlocator/items')
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('dealerlocator/adminhtml_dealerlocator_edit'))
				->_addLeft($this->getLayout()->createBlock('dealerlocator/adminhtml_dealerlocator_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('dealerlocator')->__('Item does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				$model->save();
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('dealerlocator')->__('Item was successfully saved'));
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($dealerlocatorIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($dealerlocatorIds)) {
            ...
            try {
                foreach ($dealerlocatorIds as $dealerlocatorId) {
                    $dealerlocator = Mage::getSingleton('dealerlocator/dealerlocator')
                        ->load($dealerlocatorId)
                        ->setStatus($this->getRequest()->getParam('status'))
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($dealerlocatorIds))
                );
            } catch (Exception $e) {
    ...
    public function massGeocodeAction()
    {
        ...
        if(!is_array($dealerlocatorIds)) {
            ...
            try {
                foreach ($dealerlocatorIds as $dealerlocatorId) {
                    $dealerlocator = Mage::getSingleton('dealerlocator/dealerlocator')->load($dealerlocatorId);
                    $address = urlencode($dealerlocator->address);
					$json = file_get_contents(Mage::getStoreConfig('magebuzz/google_map_options/google_geo_api_url')."?address=$address&sensor=false");
					$json = json_decode($json);
					$latitude = strval($json->{'results'}[0]->{'geometry'}->{'location'}->{'lat'});
					$longitude = strval($json->{'results'}[0]->{'geometry'}->{'location'}->{'lng'});
					$dealerlocator                      
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('dealerlocator/adminhtml_dealerlocator_grid')
            ->getCsv();
