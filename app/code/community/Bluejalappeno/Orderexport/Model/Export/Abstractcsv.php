...
abstract class Bluejalappeno_Orderexport_Model_Export_Abstractcsv extends Mage_Core_Model_Abstract
{
     ...
     */
    abstract public function exportOrders($orders, $items);
    ...
    protected function getItemUpc($item)
    {
        if ($item->getProductType() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) {
            $sku = $item->getProductOptionByCode('simple_sku');
            if(is_object(Mage::getModel('catalog/product')->loadByAttribute('sku',$sku))) {
	            return Mage::getModel('catalog/product')->loadByAttribute('sku',$sku)->getUpc();
            } else {
	            return '';
            }
        ...
        if(is_object(Mage::getModel('catalog/product')->loadByAttribute('sku',$item->getSku()))) {
	        return Mage::getModel('catalog/product')->loadByAttribute('sku',$item->getSku())->getUpc();
        } else {
	        return '';
        }
    ...
    protected function getItemCpgType($item)
    {
        if ($item->getProductType() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) {
            $sku = $item->getProductOptionByCode('simple_sku');
            if(is_object(Mage::getModel('catalog/product')->loadByAttribute('sku',$sku))) {
	            $product_type = Mage::getModel('catalog/product')->loadByAttribute('sku',$sku)->getAttributeText('cpg_product_type');
            } else {
	            $product_type = 'CL';
            }
        ...
        if(is_object(Mage::getModel('catalog/product')->loadByAttribute('sku',$item->getSku()))) {
	        $product_type = Mage::getModel('catalog/product')->loadByAttribute('sku',$item->getSku())->getAttributeText('cpg_product_type');
        } else {
	        $product_type = 'CL';
        }
