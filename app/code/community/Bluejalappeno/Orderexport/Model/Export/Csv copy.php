...
class Bluejalappeno_Orderexport_Model_Export_Csv extends Bluejalappeno_Orderexport_Model_Export_Abstractcsv
{
    ...
    {	
        ...
        foreach ($orders as $order) {
        	...
        	if($order->getStatus() == 'processing') {
	            ...
	            $order->setStatus('sent_to_warehouse');
	            $order->addStatusToHistory('sent_to_warehouse', '', false);
	            $order->save();
            }
        }
    }
    ...
    {
    	if ($handle = opendir('/home/edi/import')) {
		
		    while (false !== ($entry = readdir($handle))) {
		    	if ($entry != "." && $entry != "..") {
			    	$this->csv_file_to_mysql_table('/home/edi/import/' . $entry, 'bulk_process');
	...
	*/
	...
	public function csv_file_to_mysql_table($source_file, $target_table, $max_line_length=10000) {
	    ...
	    if (($handle = fopen("$source_file", "r")) !== FALSE) { 
	        ...
	        fclose($handle);
	        rename($source_file, '/home/edi/archive/order_import_'.date("Ymd_His").'.csv');
	    } 
	} 
...
protected function writeOrder($order, $fileName)
    {
        
        $orderItems = $order->getItemsCollection();
        
        if($orderItems) {
        	//$fp = fopen('/home/edi/export/'.$fileName, 'w');
	        ...
	        if(count($product_types) > 1) {
		        foreach($product_types as $product_type) {
		        	...
		        	$fp = fopen('/home/edi/export/' . $fileName . ' - ' . $product_type . '.txt', 'w');
		        	$total_qty_ordered = 0;
		        	$num_items = 0;
		        	$subtotal = 0;
    		        fputcsv($fp, $common, self::DELIMITER, self::ENCLOSURE);
			        ...
			        fclose($fp);
		            $file_contents = file_get_contents('/home/edi/export/'.$fileName . ' - ' . $product_type . '.txt');
		            $file_contents = str_replace('"', '' ,$file_contents);
		            $file_contents = $this->normalize($file_contents);
		            file_put_contents('/home/edi/export/'.$fileName . ' - ' . $product_type . '.txt', $file_contents);
		        }
	        } else {
		        $common = $this->getCommonOrderValues($order);
				$shipping = $this->getShippingOrderValues($order);
	        	$fp = fopen('/home/edi/export/' . $fileName . '.txt', 'w');
	        	$total_qty_ordered = 0;
	        	$num_items = 0;
	        	$subtotal = 0;
		        fputcsv($fp, $common, self::DELIMITER, self::ENCLOSURE);
		        ...
		        fputcsv($fp, array('Z', $num_items, $total_qty_ordered, number_format($subtotal, 2), '', '', '', '', ''), self::DELIMITER, self::ENCLOSURE);
		        fclose($fp);
	            $file_contents = file_get_contents('/home/edi/export/'.$fileName . '.txt');
	            $file_contents = str_replace('"', '' ,$file_contents);
	            $file_contents = $this->normalize($file_contents);
	            file_put_contents('/home/edi/export/'.$fileName . '.txt', $file_contents);
	        }
        }
    }
...
*/
    ...
    protected function getCommonOrderValues($order, $product_type = NULL)
    {
        ...
        return array(
        	'H',
            $order->getRealOrderId() . $product_type,
            $order->getRealOrderId() . $product_type,
            'Cobra Ecomm',
...
*/
	...
	public function importStock() {
		...
		if ($handle = opendir('/home/edi/inventory')) {
				
		    while (false !== ($entry = readdir($handle))) {
		    	if ($entry != "." && $entry != ".." && $entry != "archive" && strpos($entry,'Bulk') != false) {
					//echo '/home/edi/inventory/'.$entry;
					
					
					$file = fopen('/home/edi/inventory/'.$entry, 'r');
					...
					rename('/home/edi/inventory/'.$entry, '/home/edi/archive/bulk_import_'.date("Ymd_His").'.csv');
				
		        } elseif($entry != "." && $entry != ".." && $entry != "archive" && strpos($entry,'Bulk') == false && strpos($entry,'Tracking') == false) {
					...
					rename('/home/edi/inventory/'.$entry, '/home/edi/archive/inventory_import_'.date("Ymd_His").'.csv');
					
		        } elseif($entry != "." && $entry != ".." && $entry != "archive" && strpos($entry,'Bulk') == false && strpos($entry,'Tracking') == true) {
			      	
			      	$file_contents = file_get_contents('/home/edi/inventory/'.$entry);
			      	...
			      	try {
			            ...
			            rename('/home/edi/inventory/'.$entry, '/home/edi/archive/custom_tracking_'.date("Ymd_His").'.csv');
		    ...
		    foreach($zero_stock_items as $item) {
			    ...
			    if($product) {
					...
					if($product->getResource()->getAttribute('manufacturer')->getFrontend()->getValue($product) == 'Puma Golf') {
