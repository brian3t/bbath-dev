...
class Fishpig_Wordpress_Helper_Router extends Fishpig_Wordpress_Helper_Abstract
{
	...
	public function isCategoryUri($uri)
	{
		if (!$this->categoryUrlHasBase()) {
			if ($this->_isFullCategoryUri($uri)) {
				$base = $this->getCategoryBase();

				if (substr($uri, 0, strlen($base)) == $base) {
					$newUri = trim(substr($uri, strlen($base)), '/');

					$category = Mage::getModel('wordpress/post_category')->loadBySlug($newUri);
				...
				if ($category->getId()) {
					Mage::register('wordpress_category', $category, true);
					return true;
				}
			}
		...
		return $this->_isFullCategoryUri($uri);
	}
	
	/**
	 * Check whether the uri matches the category base pattern
	 *
	 * @param string $uri
	 * @return bool
	 */
	protected function _isFullCategoryUri($uri)
	{
		return preg_match('/^' . str_replace('/', '\/', $this->getCategoryBase()) . '\/' . $this->getPermalinkStringRegex() . '$/i', $uri);
	}
