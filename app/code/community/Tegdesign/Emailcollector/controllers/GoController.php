    ...
    public function addOrUpdateToBronto($email, $fields, $generated_coupon = NULL) {
	    ...
	    if(!empty($fields)) {
		    ...
		    foreach($fields as $key => $value) {
			    if($value != '' && $key != 'popup_email' && $key != 'ok' && $key != 'store_id' && $key != 'year' && $key != 'month' && $key != 'day') {
	    ...
	    if($generated_coupon != '') {
		    $bronto_fields[] = array('fieldId' => '0bce03e90000000000000000000000017054', 'content' => $generated_coupon); //coupon_code
		    $bronto_fields[] = array('fieldId' => '0bce03e90000000000000000000000017809', 'content' => date('Y-m-d')); // coupon_date
...
*/
		...
		try {
			...
			if ($write_result->errors) {
				...
				$this->debugLog('request', printRequestResponse($client), false);
				...
				$this->debugLog('request/response', $client->__getLastRequest() . $client->__getLastResponse(), false);
...
*/
    ...
    public function printRequestResponse($client) {
		$response = 'Request' . "\n";
		$response .= htmlspecialchars($client->__getLastRequest());  
		$response .= "\n";
	...
	public function printFault($exception, $client) {
	    $response = "<b>Code:</b>{$exception->faultcode}<br>\n";
	    $response .= "<b>String:</b>{$exception->faultstring}<br>\n";
	    return $response;
	}
