...
var lookupTitle = function(titleToLook4, ele){
	...
	ele.each(function(e){
		...
		title = title.trim(); 
		title = title.slice(0 - titleToLook4.length);
		if (levenshtein(title, titleToLook4) < leastDistance){
			//only assign if title is close enough
			if ((levenshtein(title, titleToLook4) / titleToLook4.length) < 0.5) {
				result = this;	
			} 
...
jQuery(document).ready(function() {
	...
	var updateImage = function(e) {
			...
			var matchedEle = lookupTitle(selColor, 'a.cloud-zoom-gallery');
			if (matchedEle) {
				matchedEle.click();
				}
		};
