	...
	{	
		if (!isset($this->_cache['is_adminhtml_area'])) {
			if (Mage::getDesign()->getArea() == 'adminhtml') {
				...
				$currentUrl = strtolower(Mage::helper('core/url')->getCurrentUrl());
				$adminFrontName = Mage::getSingleton('core/config')->init()->getXpath('admin/routers/adminhtml/args/frontName');
		
				if (isset($adminFrontName[0])) {
					$this->_cache['is_adminhtml_area'] = (strpos($currentUrl, '/index.php/' . $adminFrontName[0] . '/') !== false);
				}
				else {
	...
	protected function _getStoreForConfig()
	{
		if ($this->isAdminhtmlArea()) {
			if (!isset($this->_cache['current_store'])) {
				...
				else if (preg_match("/\/store\/([a-z0-9\-\_]{1,})\//", Mage::helper('core/url')->getCurrentUrl(), $results)) {
					$this->_cache['current_store'] = $results[1];
				}
				else if ($website) {
					if (preg_match("/\/website\/([a-z0-9\-\_]{1,})\//", Mage::helper('core/url')->getCurrentUrl(), $results)) {
